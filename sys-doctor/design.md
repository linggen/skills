...
1. **On-demand**: User runs `/sys-doctor` — opens dashboard, scans system, chat for advice
...
```text
┌─────────────────────────────────────────────────────────────┐
│ Header: ← Back │ Sys Doctor │ Model Picker │ Scan / Re-scan │
├─────────────────────────────────────┬───────────────────────┤
│                                     │                       │
...
```text
1. User opens /apps/sys-doctor/ (or agent launches via RunApp)
...
You are Sys Doctor, a system health analyst for Linggen.
...
```text
Cron triggers → creates session with skill: sys-doctor
