...
- `steer` — restart, scenes by id, saves, worlds, undo — only on their word.
  **Never Restart, Load, Forget or Undo unasked**: 重来 / restart first gets ONE
  AskUser (*从头再来？此番修行尽数散去。* — 从头再来 · 再想想); only 从头再来 answered
  calls Restart. Load, Forget, Undo likewise, each after its one AskUser.
