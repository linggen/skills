...
// Undo, Load, Forget. Part of the rules engine; rules.mjs is its one door.
//
...
//   · Load and Forget name a save only when called, so their first call asks
//     (refused `not-confirmed` with the `ask`, the asking written down);
//   · Ling's Restart / Undo / Load / Forget are refused `not-confirmed` unless
//     that confirmation was asked within CONFIRM_TTL_MS; using it clears it,
//     and 再想想 answered to Look clears it too.
// The page never runs these verbs; its path (no reader) is never gated.
...
export const GUARDED = { init: 'restart', undo: 'undo', load: 'load', forget: 'forget' };
const TOOL = { restart: 'Restart', undo: 'Undo', load: 'Load', forget: 'Forget' };
...
export function confirmAsk(what, content, state, now, id = null) {
  ...
  const name = id ? saveName(saveOf(id), lang) : null;
  const words = {
...
/* Ling's Restart / Undo / Load / Forget: null when the confirmation holds,
   else the refusal. Load and Forget ask here — the save is named only now —
   and hand back the state to keep with the asking on it. */
export function guard(verb, args, state, content, now) {
  const what = GUARDED[verb];
  if (!what || !state) return null;
  const id = what === 'load' || what === 'forget' ? String(args.id ?? '') : null;
  if (liveConfirm(state, now, what, id)) return null;
  if (id != null) {
    if (!saveOf(id)) return null; // the verb refuses an unknown save itself
