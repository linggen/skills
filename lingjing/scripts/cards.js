...
export const WORDS = {
  zh: {
    ...
    uncast: '今日未卜', uncastHint: '一日一卦，问今日斗法的手气。请银月三钱六掷。', cast: '起一卦', sayCast: '请银月起一卦', throwing: '起卦中……', changedTo: '之卦',
    effEven: '今日斗法无增无减', effSpell: '今日斗法，{root}法术 {n}', effSight: '看得出妖下回合的架势',
    ...
    qi: '体力', qiFull: '充盈', qiHalf: '半满', qiLow: '将尽', qiEmpty: '已空', qiBackAt: '{t} 回满',
    emptyLine: '体力耗尽了。回到现实里歇一歇——起身走走，喝口水。{t} 回满，再出发。', emptySoon: '体力耗尽了。回到现实里歇一歇——起身走走，喝口水。随时辰恢复。',
    boardsStay: '坊市与交差照常。',
  ...
  en: {
    ...
    uncast: 'Not yet cast today', uncastHint: "One reading a day: today's luck in a fight. Yinyue throws three coins, six times.", cast: 'Cast the coins', sayCast: 'Yinyue, cast the coins for me', throwing: 'Casting…', changedTo: 'Changing to',
    effEven: 'Fights today: no gain, no loss', effSpell: 'Fights today: {root} spells {n}', effSight: "you read the beast's next move",
    ...
    qi: 'Stamina', qiFull: 'full', qiHalf: 'half', qiLow: 'low', qiEmpty: 'empty', qiBackAt: 'full at {t}',
...
/// the birthday typed here — read by the page on this machine, never said in
/// the chat — or a random one, or none; a declined one can still be set.
...
function hexagram(card, ctx) {
  ...
  // Before the day's reading: the coins wait, one tap casts them — the page
  // casts with the rules and 银月 reads it (his, 2026-09-23: 既然是请银月,
  // 需要银月给结果). Nothing is asked first: it is always the day's fight luck
  // (问卦, redesign-v2). No word to Ling, no waiting on her turn.
  if (!d) {
    return `<div class="card hex uncast"><div class="coins">${'<i></i>'.repeat(3)}</div><div>
      <div class="cardtitle">${esc(w.uncast)}</div><div class="hextext">${esc(w.uncastHint)}</div><div class="acts"><button class="act" data-divine>${esc(w.cast)}</button></div>${fateHtml(ctx)}</div></div>`;
  }
...
function castHtml(d, ctx) {
  ...
  const e = d.effect ?? {};
  const lines = [];
  if (e.card) lines.push(say(w.effSpell, { root: e.root?.name ?? '', n: e.card > 0 ? `+${e.card}` : e.card }));
  if (e.sight) lines.push(w.effSight);
  ...
  return `<div class="card hex cast${ctx.castFresh ? ' casting' : ''} ${esc(d.grade.id)}"><div class="yaos">${rows}</div><div>
    ...
    <div class="small castfx">${esc(effect)}</div>${fateHtml(ctx)}
...
export const liftSaid = (e, w) => Object.entries(e?.lift ?? {}).map(([k, n]) => (w.effLift?.[k] ? say(w.effLift[k], { n }) : '')).filter(Boolean).join(' · ');
...
const DOES = {
  ...
  core: (e, w) => say(w.effCore, { root: e.core_name }),
  charm: (e, w) => w.effCharm,
  key: (e, w) => w.effKey,
  pouch: (e, w) => say(w.effPouch, { n: e.pouch }),
};
export function itemDoes(effect, ctx) {
...
export function tamesLine(i, ctx) {
...
/// amplifies. It grows with the story, never from a tap (redesign-v2 § 四):
/// the card says so, once, until it is at its ninth.
function treasure(card, ctx) {
  ...
  return `<div class="card treasurecard"><div class="cardtitle">${esc(w.treasureTitle)}</div>
    ...
    <div class="small ling">${esc(t.top ? w.treasureTop : w.treasureGrows)}</div>
    <div class="acts"><button class="act ask" ${askAttr(say(ctx.words.sayTreasure, { name: t.name }))}>${esc(w.about)}</button></div></div>`;
}
...
function seclusion(card, ctx) {
  ...
  if (!e) return '';
  const t = e.target ?? {}, what = t.name ?? w.secludeFoci[e.focus];
...
const SECLUDE_LEVEL = {
  card: (t) => ` <span class="stars">${'★'.repeat(t.star ?? 0)}${'☆'.repeat(Math.max(0, (t.top ?? 3) - (t.star ?? 0)))}</span>`,
  treasure: (t) => (t.step ? ` <span class="small dim">${esc(t.step)}</span>` : ''),
  progress: () => '',
};
const secludeLevel = (focus, t, w) => (SECLUDE_LEVEL[focus] ?? SECLUDE_LEVEL.progress)(t, w);

/* The bar toward the next step: hours into it of the hours it takes, what it
   brings, and the clock hour it lands (none past the 12-hour cap). */
const SECLUDE_NEXT = {
  card: (t, w) => (t.done ? w.secludeTop : say(w.secludeBarStar, { what: t.next === 'cost' ? say(w.secludeNextCost, { was: t.cost, cost: t.cost - 1 }) : w.secludeNextPower })),
  treasure: (t, w) => (t.done ? w.secludeTop : w.secludeBarTreasure),
  progress: (t, w) => say(w.secludeBarXw, { n: t.per_hour }),
};
function secludeBar(focus, t, w, ctx) {
  if (!t.need) return '';
  const pct = focus === 'progress' ? 100 : Math.round((Math.min(t.into ?? 0, t.need) / t.need) * 100);
  const hours = focus === 'progress' ? '' : `${t.into}/${t.need} ${w.hourUnit} · `;
  const at = t.at && focus !== 'progress' ? ` · ${say(w.secludeAt, { t: clockOf(new Date(t.at), ctx.lang) })}` : '';
  return `<div class="secludebar"><i style="width:${pct}%"></i></div><div class="small">${esc(hours)}${esc((SECLUDE_NEXT[focus] ?? SECLUDE_NEXT.progress)(t, w))}${esc(at)}</div>`;
}
...
function chanceRow(ctx) {
  ...
  if (!left) return '';
  // What it is and one tap to it (2026-09-25, "这个是什么, 不能点呢"): the
  // row said only a place and a clock. There, the stage holds its 收下.
  const go = c.here ? '' : goBtn(say(w.go, { name: c.place.name }), c.place.id);
  return `<div class="bookrow chance${left <= 30 ? ' soon' : ''}"><div><b>${esc(say(w.chanceTitle, { place: c.place.name }))}</b>
    <span class="small dim">${esc(c.here ? w.chanceHere : say(w.chanceLeft, { t: hm(left, w) }))}</span>
    <div class="small dim">${esc(c.here ? w.chanceLine : w.chanceWhat)}</div></div>${go}</div>`;
}

export function bookChipHtml(ctx, open, fresh) {
  const book = ctx.look?.book ?? [], w = ctx.words, lucky = chanceLeft(ctx.look?.chance) ? 1 : 0;
  if (!book.length && !lucky && !ctx.look?.waypoint && !ctx.look?.kaifu) return '';
  // A 开府 milestone done and unpaid is one more to hand in — the chip says so.
  const ready = book.filter((q) => q.ready).length + (ctx.look?.kaifu?.ready ?? 0);
...
export function bookPopHtml(ctx) {
  ...
  return `<div class="bookpop" role="dialog">${head}${bookHtml(ctx) || (g ? '' : `<div class="small dim">${esc(w.bookNone)}</div>`)}${work}${kaifuHtml(ctx)}</div>`;
}

/// 储物袋 — the top bar's chip. What it opens is the full panel over the
/// stage (pouch.js; Hanli, 2026-09-25): what he wears above, what he carries
/// below, from the rules' `gear` read fetched on the tap — it never rides Look.
export function gearChipHtml(ctx, open) {
  if (!ctx.look?.traits) return '';
  return `<span class="bookwrap"><button class="bookchip gearchip" data-gear aria-expanded="${open ? 'true' : 'false'}">${esc(ctx.words.gearChip)}</button></span>`;
}

/* What the fight takes from what he wears (rules § 装备入局), one line:
   主灵根一击 +n · 护体 n · 抗土 n · 符在手 · 借金. */
export function gearFightHtml(f, ctx) {
  if (!f) return '';
...
/* Her card as it stands (rules companion.mjs herCard; Hanli, 2026-09-24):
   攻/血 and the realm it grew with, then each ability a cauldron gave her —
   a line each. */
const CN_NUM = ['〇', '一', '二', '三', '四', '五', '六', '七', '八', '九'];
export function herCardHtml(her, ctx) {
  const c = her.card, w = ctx.words;
  if (!c) return '';
  const realm = c.realm && (c.realm.atk || c.realm.hp) ? ` · ${say(w.herRealm, { realm: c.realm.name, atk: c.realm.atk, hp: c.realm.hp })}` : '';
  const gifts = c.gifts.map((g) => `<div class="small">${esc(say(w.herGift, { n: ctx.lang === 'en' ? g.found : CN_NUM[g.found] ?? g.found, name: g.name, does: g.does }))}</div>`).join('');
  return `<div class="gearher"><div class="small dim">${esc(say(w.herCard, { name: her.name, atk: c.atk, hp: c.hp }))}${esc(realm)}</div>${gifts}</div>`;
}

/// 牌组 — every card he holds, the day's ten lit (得牌). From 结丹 on each
/// card is a tap — lit is in the ten; his own picks solid, the rules' fill
/// dashed (his, 2026-09-23). Before, the roots deal the ten and nothing is a
/// button (redesign-v2 § 四). 银月 and a root he lacks stay put. The pouch
/// panel opens it from its 牌组 button.
export function deckHtml(g, ctx) {
  const held = g.cards ?? [], w = ctx.words;
  if (!held.length) return '';
  const mine = held.filter((c) => c.picked).length;
  const chip = (c) => {
  ...
  const short = 10 - mine;
  const note = g.picking ? (short > 0 ? say(w.cardsShort, { mine, short }) : say(w.cardsPicked, { mine })) : g.can_pick ? w.cardsNote : w.cardsDealt;
  return `<div class="cardtitle bagtitle">${esc(say(w.cardsTitle, { n: held.length }))}</div>
...
function bookHtml(ctx) {
  ...
  const rows = book.map((q) => {
    ...
    const chev = `<span class="chev" aria-hidden="true">${open ? '▾' : '▸'}</span>`;
    // A 人间功课 line: where it is done (💻/📱) and its app's page, one tap away.
    const act = q.ready ? doBtn(w.turnIn, 'turn', q.id) : q.chore?.open ? `${openBtn(q.chore, w)}${chev}` : chev;
    const mark = q.chore ? deviceMark(q.chore.device) : '';
    return `<div class="bookrow${q.ready ? ' ready' : ''}${open ? ' open' : ''}" data-bookrow="${esc(q.id)}" role="button" tabindex="0" aria-expanded="${open}"><div>${mark ? `<span class="device">${esc(mark)}</span> ` : ''}<b>${esc(q.title)}</b>
...
/* 💻 · 📱 — where a chore is done; `both` (or unsaid) wears both. */
const DEVICE_MARK = { mac: '💻', phone: '📱', both: '💻📱' };
const deviceMark = (d) => DEVICE_MARK[d] ?? '';
const openBtn = (c, w) => `<a class="act" href="${esc(c.open)}" target="_blank" rel="noopener">${esc(say(w.choreOpen, { app: appName(c.app) }))} ↗</a>`;

/// 开府 — the one-time setup milestones, in their own section (Quest
/// `kaifu`, read by the page when the chip opens): ✓ when its app saw it,
/// 交差 once when done and unpaid, else the page where it is done. Never a
/// nag — it is only here.
function kaifuHtml(ctx) {
  const list = ctx.kaifu ?? [], w = ctx.words;
  if (!list.length) return '';
  const row = (k) => {
    const act = k.paid ? `<span class="small dim">${esc(w.kaifuPaid)}</span>` : k.done ? doBtn(w.turnIn, 'turn', k.id) : k.open ? openBtn(k, w) : '';
    return `<div class="bookrow kaifu${k.done ? ' done' : ''}${k.done && !k.paid ? ' ready' : ''}"><div><span class="tick">${k.done ? '✓' : '○'}</span> <span class="device">${esc(deviceMark(k.device))}</span> <b>${esc(k.title)}</b></div>${act}</div>`;
  };
  // What is counted is folded away (his, 2026-09-24: a finished 开府 need not
  // stand there for good) — only what is still open, or waits for 交差, shows.
  const paid = list.filter((k) => k.paid), open = list.filter((k) => !k.paid);
  const head = say(w.kaifuTitle, { n: list.filter((k) => k.done).length, of: list.length });
  const fold = paid.length ? `<details class="kaifufold"><summary class="small dim">${esc(open.length ? head : `${head} · ${w.kaifuDone}`)}</summary>${paid.map(row).join('')}</details>` : '';
  if (!open.length) return `<div class="book kaifubook">${fold}</div>`;
  return `<div class="book kaifubook"><div class="small dim">${esc(head)}</div>${open.map(row).join('')}${fold && `<details class="kaifufold"><summary class="small dim">${esc(say(w.kaifuPaidN, { n: paid.length }))}</summary>${paid.map(row).join('')}</details>`}</div>`;
}
...
function bookDetail(q, ctx) {
  ...
  return `<div class="bookdetail">
    ...
    <div class="acts">${q.where && !q.where.here ? goBtn(say(w.go, { name: q.where.name }), q.where.id) : ''}${askBtn(w.about, say(w.sayQuestAbout, { title: q.title }))}${i.kind === 'errand' || i.kind === 'tale' ? `<button class="act quiet" data-drop="${esc(q.id)}">${esc(w.drop)}</button>` : ''}</div></div>`;
}
...
/// nothing while it is veiled (the page plays a mist over the stage and
/// reveals it itself — lingjing.js watchVeil; a 抉择 waits for Ling's offer);
...
function road(card, ctx) {
  ...
  if (!onRoad(m)) return '';
  return ROAD_CARD[m.kind]?.(m, ctx.words) ?? '';
}
