...
| 炉石 | 我们 | 带什么 |
|---|---|---|
| 随从 | **灵兽 · 同道** | 攻/血、五行、关键词：护主 (嘲讽) · 入阵 (战吼) · 遗蜕 (亡语) · 疾行 (冲锋) |
| 法术 | **功法** | 一次性，有五行：伤害、清场、回气血、抽牌 |
| 武器 | **法器** | 攻击力 + 耐久；本命法宝是它的顶端 |
| — | **符 · 丹** | 消耗品：用掉就真的从袋里少一张。实物，这一条是我们独有的 |
| 英雄技能 | **主灵根一击** | 每回合一次，费 2，打出主根那一行 |

**银月是一张随从**，3/4，入阵回 2 气血；每局开局就在手上，**不占牌库的 12 张**；
被打到 0 是退到一边，下一局照来 (2026-09-18, his: *Yinyue是一个随从*)。

### 一局的数字（第一版，闸会调）

气血 练气 20 · 筑基 26 · 结丹 32 · 元婴 40 ·
灵力起 1，每回合 +1，上限 6 / 8 / 10 / 10，每回合回满 ·
牌库 12 · 起手 4（可换牌一次）· 每回合抽 1 · 阵前 3 位 ·
抽空是**反噬**：每次抽牌掉 1、2、3… 气血。

**五行**：牌的行克对方的根 ×2、被克 ×½、无关 ×1；灵兽互相攻击时同样算。
第一版**只上相克**；相生减费（「势」）留到玩家打顺之后 —— 多人要公平，规则先薄。
...
| 牌库 | 你 12 · 妖 **8，抽空即力竭退走** | 各 12 | 各 12 |
| 先后手 | 你先手，多抽一张 | 战力定先手，后手补一张 | 按战力轮转 |

**境界压制（只在 PvE）**：高一境 你 ×1.5 / 它 ×0.7；高两境 ×2 / ×0.5；低一境反过来
—— 越级有风险也有彩头。**妖的 8 张硬上限**保证日常一场绝不拖过八回合 (2026-09-18, his 加)。

**PvP 为什么不带境界**：公平只能来自同一条灵力曲线和同样 12 张。境界只决定你能带
...
倒计时给人压力，体力不给 (2026-09-18, his)。丹田那个环显示**点数**，不是时间：
一场战斗几点、一个场景几点。**次数不再限制**，一天能打好几只妖，只要体力够；
但**同一只妖一天只应一次**（它退进雾里）。每日回满，**现实里的事回体力**
（Shifu 扫盘、Health 走够步数）—— 这是别的游戏抄不走的钩子。每日修为与灵石的
上限本来就在，所以放开次数不会失控。
