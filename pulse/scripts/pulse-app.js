...
async function runGatherWeb() {
  ...
  const goal = [
    ...
    '  2. PICK A REPLY TARGET. From the comments FetchRedditThread returned, pick the single best one to engage: high-signal, raises a specific question or makes a claim my distinct angle would complement, not already well-addressed. If you find one, emit `reply_target: { author: "u/<handle>", body: "<verbatim comment text>", score: <int>, age_hours: <int>, depth: <int>, url: "<full reddit permalink to this comment>" }`. If no comment clears the bar, OR the comments weren\'t available, OR the OP itself is the strongest hook, leave `reply_target` absent and draft to the OP.',
    '  3. GROUND THE DRAFT. Use the OP body + whatever comments FetchRedditThread returned (at minimum the OP). Don\'t parrot existing commenters; offer a distinct angle. If `reply_target` is set, `draft_starter` is the reply TO that comment, not the OP.',
