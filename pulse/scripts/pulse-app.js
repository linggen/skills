...
async function runDraft() {
  ...
  const goal = [
    ...
    '',
    'For each enabled lane in config.targets[*].enabled, generate one draft following references/lane-templates.md constraints. Four passes — do NOT skip:',
    '',
    'Pass 1 — Concrete claim. Pick ONE specific thing from the cards: a number, a file path, a function name, a before/after, a metric. The draft\'s thesis must be that thing, not a principle about it. If the only thing you can come up with is an axiom ("X should not Y", "the boundary between A and B"), the draft is not ready — emit an `empty` card instead.',
    '',
